SynthiqBotsUI.CLEAR = function(pString, pAmount, o1, o2, o3)
	for i = 1, pAmount, 1 do
		if(o1 == nil) then
			pString = SynthiqBotsUI.doReplace(pString, "|cff%w%w%w%w%w%w", "")
			pString = SynthiqBotsUI.doReplace(pString, "|h", "")
			pString = SynthiqBotsUI.doReplace(pString, "|r", "")
		else
			if(o1 ~= nil) then pString = SynthiqBotsUI.doReplace(pString, o1, "") end
			if(o2 ~= nil) then pString = SynthiqBotsUI.doReplace(pString, o1, "") end
			if(o3 ~= nil) then pString = SynthiqBotsUI.doReplace(pString, o1, "") end
		end
	end
...
SynthiqBotsUI.CASE = function(pCondition, pDefault, oCase1, oCase2, oCase3, oCase4, oCase5, oCase6, oCase7, oCase8, oCase9)
...
SynthiqBotsUI.IF = function(pCondition, pSuccess, pFailure)
	if(pCondition) then return pSuccess else return pFailure end
end

SynthiqBotsUI.doSlash = function(pCommand, pArguments)
	...
	SendChatMessage(SynthiqBotsUI.info.command, "SAY")
	return false
end

SynthiqBotsUI.doDot = function(pCommand, oArguments)
	SendChatMessage(pCommand .. " " .. oArguments)
	return false
end

SynthiqBotsUI.doDotWithTarget = function(pCommand, oArguments)
	...
	SendChatMessage(SynthiqBotsUI.info.target, "SAY")
	return false
end

SynthiqBotsUI.doSplit = function(pString, pPattern)
	local tResult = {}
	local tStart = 1
...
SynthiqBotsUI.doReplace = function(pString, pSearch, pReplace)
...
SynthiqBotsUI.doRemove = function(pIndex, pName)
...
SynthiqBotsUI.doRepos = function(pIndex, pOffsetX)
	local tButton = SynthiqBotsUI.frames["MultiBar"].buttons[pIndex]
	local tFrame = SynthiqBotsUI.frames["MultiBar"].frames[pIndex]
	if(tButton == nil) then tButton = SynthiqBotsUI.frames["MultiBar"].frames["Left"].buttons[pIndex] end
	if(tFrame == nil) then tFrame = SynthiqBotsUI.frames["MultiBar"].frames["Left"].frames[pIndex] end
	if(tButton == nil) then tButton = SynthiqBotsUI.frames["MultiBar"].frames["Right"].buttons[pIndex] end
	if(tFrame == nil) then tFrame = SynthiqBotsUI.frames["MultiBar"].frames["Right"].frames[pIndex] end
...
SynthiqBotsUI.isActive = function(pName)
	for key, value in pairs(SynthiqBotsUI.index.actives) do if(value == pName) then return true end end
	return false
end

SynthiqBotsUI.isInside = function(pString, p1stPattern, o2ndPattern, o3rdPattern, o4thPattern, o5thPattern, o6thPattern, o7thPattern, o8thPattern, o9thPattern)
...
SynthiqBotsUI.beInside = function(pString, p1stPattern, o2ndPattern, o3rdPattern, o4thPattern, o5thPattern, o6thPattern, o7thPattern, o8thPattern, o9thPattern)
	if(pString == nil) then return false end
...
SynthiqBotsUI.isRoster = function(pRoster, pName)
	for key, value in pairs(SynthiqBotsUI.index[pRoster]) do if(pName == value) then return true end end
	return false
end

SynthiqBotsUI.isMember = function(pName)
...
SynthiqBotsUI.isTarget = function()
	local tName = UnitName("target")
...
SynthiqBotsUI.isUnit = function(pUnit)
...
SynthiqBotsUI.toClass = function(pClass)
	local pLower = string.lower(pClass)
	local pStart = string.sub(pLower, 1, 5)
	
	for i = 1, 10 do
		local tOutput = SynthiqBotsUI.data.classes.output[i]
		local tInput = SynthiqBotsUI.data.classes.input[i]
...
SynthiqBotsUI.toUnit = function(pName)
	if(GetNumRaidMembers() > 5) then
		for i = 1, GetNumRaidMembers() do
...
SynthiqBotsUI.toTip = function(pClass, pLevel, pName)
	local tTip = pClass .. " - "
	if(pLevel ~= nil) then tTip = tTip .. pLevel .. " - " end
	tTip = tTip .. pName .. SynthiqBotsUI.tips.unit.button
	tTip = SynthiqBotsUI.doReplace(tTip, "NAME", pName)
	tTip = SynthiqBotsUI.doReplace(tTip, "NAME", pName)
	tTip = SynthiqBotsUI.doReplace(tTip, "NAME", pName)
	return tTip
end

SynthiqBotsUI.toPoint = function(pFrame)
	local tX = pFrame:GetRight()
	local tY = pFrame:GetBottom()
	local tResolution = SynthiqBotsUI.doSplit(({ GetScreenResolutions() })[GetCurrentResolution()], "x")
	local tHeight = tonumber(tResolution[2])
	local tWidth = tonumber(tResolution[1])
	local tScale = 1 / tWidth * SynthiqBotsUI:GetRight()
	return math.floor(tX - (tWidth * tScale)), math.floor(tY)
end

SynthiqBotsUI.RaidPool = function(pUnit, oWho)
	if(pUnit ~= "player" and SynthiqBotsUI.getBot(pUnit) == nil) then return end
	
	local tGender = SynthiqBotsUI.CASE(UnitSex(pUnit), "[U]", "[N]", "[M]", "[F]")
	...
	if(oWho ~= nil) then
		local tWho = SynthiqBotsUI.CLEAR(oWho, 20)
		tWho = SynthiqBotsUI.doReplace(tWho, "beast mastery", "Beast-Mastery")
		tWho = SynthiqBotsUI.doReplace(tWho, "feral combat", "Feral-Combat")
		tWho = SynthiqBotsUI.doReplace(tWho, "Blood Elf", "Blood-Elf")
		tWho = SynthiqBotsUI.doReplace(tWho, "Night Elf", "Night-Elf")
		
		tParts = SynthiqBotsUI.doSplit(tWho, ", ")
		tSpace = SynthiqBotsUI.doSplit(tParts[1], " ")
		tScore = SynthiqBotsUI.doSplit(tParts[2], " ")[1]
		
		if(SynthiqBotsUI.isInside(tSpace[5], "/")) then tIndex = { 5, 6, 7 } else
		if(SynthiqBotsUI.isInside(tSpace[6], "/")) then tIndex = { 6, 7, 8 } else
		if(SynthiqBotsUI.isInside(tSpace[7], "/")) then tIndex = { 7, 8, 9 }
		end end end
		
		tTabs = SynthiqBotsUI.doSplit(strsub(tSpace[tIndex[1]], 2, strlen(tSpace[tIndex[1]]) - 1), "/")
		
		if(tGender == nil) then tGender = tSpace[2] end
		if(tClass == nil) then tClass = SynthiqBotsUI.toClass(tSpace[tIndex[2]]) end
		if(tRace == nil) then tRace = tSpace[1] end
		if(tName == nil) then tName = pUnit end
		if(tLevel == nil) then tLevel = substr(SynthiqBotsUI.doSplit(tSpace[tIndex[3]], " ")[1], 2) end
	else
		tScore = SynthiqBotsUI.ItemLevel(pUnit)
		tTabs[1] = GetNumTalents(1)
		tTabs[2] = GetNumTalents(2)
		tTabs[3] = GetNumTalents(3)
	end
	 
	local tTabIndex = SynthiqBotsUI.IF(tTabs[3] > tTabs[2] and tTabs[3] > tTabs[1], 3, SynthiqBotsUI.IF(tTabs[2] > tTabs[3] and tTabs[2] > tTabs[1], 2, 1))
	local tSpecial = SynthiqBotsUI.CLEAR(SynthiqBotsUI.info.talent[SynthiqBotsUI.toClass(tClass) .. tTabIndex], 1)
	
	if(tLocalClass == nil) then tLocalClass = tClass end
	if(tLocalRace == nil) then tLocalRace = tRace end
	
	SynthiqBotsUIGlobalSave[tName] =  tLocalRace .. "," .. tGender .. "," .. tSpecial .. "," .. tTabs[1] .. "/" .. tTabs[2] .. "/" .. tTabs[3] .. "," .. tLocalClass .. "," .. tLevel .. "," .. tScore
end

SynthiqBotsUI.ItemLevel = function(pUnit)
...
SynthiqBotsUI.SavePortal = function(pButton)
	local tSave = SynthiqBotsUI.IF(pButton.goMap == nil, "", pButton.goMap)
	...
	tSave = tSave .. ";" .. SynthiqBotsUI.IF(pButton.state, 1, 0)
	return tSave
end

SynthiqBotsUI.LoadPortal = function(pButton, pValue)
	local tValue = SynthiqBotsUI.doSplit(pValue, ";")
...
SynthiqBotsUI.SpellToMacro = function(pName, pSpell, pTexture)
	local tGlobal, tAmount = GetNumMacros()
	
	if(pSpell == nil or pSpell == 0) then return SendChatMessage(SynthiqBotsUI.info.spell, "SAY") end
	if(tAmount == 18) then return SendChatMessage(SynthiqBotsUI.info.macro, "SAY") end
	
	local tMacro = string.sub(pName, 1, 14) .. tAmount
	local tSpell, tIcon, tBody = GetMacroInfo(tMacro)
	
	if(tSpell == nil) then CreateMacro(tMacro, SynthiqBotsUI.spellbook.icons[pTexture], "/t " .. pName .. " cast " .. pSpell, true) end
	PickupMacro(tMacro)
end

SynthiqBotsUI.ActionToTarget = function(pAction, oTarget)
	local tName = SynthiqBotsUI.IF(oTarget == nil, UnitName("target"), oTarget)
...
SynthiqBotsUI.ActionToTargetOrGroup = function(pAction)
	local tName = UnitName("target")
	...
	SendChatMessage(SynthiqBotsUI.info.neither, "SAY")
	return false
end

SynthiqBotsUI.ActionToGroup = function(pAction)
	if(GetNumRaidMembers() > 5) then
		SendChatMessage(pAction, "RAID")
		return true
	end
...
SynthiqBotsUI.SelectToTarget = function(pParent, pIndex, pTexture, pAction, oTarget)
	if(SynthiqBotsUI.ActionToTarget(pAction, oTarget)) then
...
SynthiqBotsUI.SelectToTargetButton = function(pParent, pIndex, pTexture, pAction, oTarget)
	local tFrame = pParent.frames[pIndex]
	local tButton = pParent.buttons[pIndex]
	tButton.doLeft = function(pButton) SynthiqBotsUI.ActionToTarget(pAction, oTarget) end
	tButton.setTexture(pTexture)
	tFrame:Hide()
	return true
end

SynthiqBotsUI.SelectToGroupButtonWithTarget = function(pParent, pIndex, pTexture, pAction)
	local tFrame = pParent.frames[pIndex]
	local tButton = pParent.buttons[pIndex]
	tButton.doLeft = function(pButton) if(SynthiqBotsUI.isTarget()) then SynthiqBotsUI.ActionToGroup(pAction) end end
	tButton.setTexture(pTexture)
	tFrame:Hide()
	return true
end

SynthiqBotsUI.SelectToGroupButton = function(pParent, pIndex, pTexture, pAction)
	local tFrame = pParent.frames[pIndex]
	local tButton = pParent.buttons[pIndex]
	tButton.doLeft = function(pButton) SynthiqBotsUI.ActionToGroup(pAction) end
	tButton.setTexture(pTexture)
	tFrame:Hide()
	return true
end

SynthiqBotsUI.SelectToGroup = function(pParent, pIndex, pTexture, pAction)
	if(SynthiqBotsUI.ActionToGroup(pAction)) then
		local tFrame = pParent.frames[pIndex]
		local tButton = pParent.buttons[pIndex]
		tButton.setTexture(pTexture)
		tFrame:Hide()
...
SynthiqBotsUI.Select = function(pParent, pIndex, pTexture)
	local tFrame = pParent.frames[pIndex]
	local tButton = pParent.buttons[pIndex]
	tButton.setTexture(pTexture)
	tFrame:Hide()
...
SynthiqBotsUI.ShowHideSwitch = function(pFrame)
...
SynthiqBotsUI.OnOffActionToTarget = function(pButton, pOn, pOff, pTarget)
	if(pButton.state) then
		SynthiqBotsUI.ActionToTarget(pOff, pTarget)
		pButton.setDisable()
		return false
	else
		SynthiqBotsUI.ActionToTarget(pOn, pTarget)
		pButton.setEnable()
		return true
	end
end

SynthiqBotsUI.OnOffSwitch = function(pButton)
	if(pButton.state) then
		pButton.setDisable()
		return false
	end
...
SynthiqBotsUI.newFrame = function(pParent, pX, pY, pSize, oWidth, oHeight, oAlign)
	local frame = CreateFrame("Frame", nil, pParent)
	frame:SetPoint(SynthiqBotsUI.IF(oAlign ~= nil, oAlign, "BOTTOMRIGHT"), pX, pY)
	frame:Show()
	...
	frame.height = SynthiqBotsUI.IF(oHeight ~= nil, oHeight, pSize)
	frame.width = SynthiqBotsUI.IF(oWidth ~= nil, oWidth, pSize)
	frame.align = SynthiqBotsUI.IF(oAlign ~= nil, oAlign, "BOTTOMRIGHT")
	frame.size = pSize
	frame.x = pX
	frame.y = pY
	...
	frame.addTexture = function(pTexture)
		...
		frame.texture:SetTexture(SynthiqBotsUI.IF(string.sub(pTexture, 1, 9) ~= "Interface", "Interface/Icons/", "") .. pTexture)
	...
	frame.wowButton = function(pName, pX, pY, pWidth, pHeight, pSize)
		...
		frame.buttons[pName] = SynthiqBotsUI.wowButton(frame, pName, pX, pY, pWidth, pHeight, pSize)
		return frame.buttons[pName]
	end
	
	frame.addButton = function(pName, pX, pY, pTexture, pTip, oTemplate)
		if(frame.buttons[pName] ~= nil) then frame.buttons[pName]:Hide() end
		frame.buttons[pName] = SynthiqBotsUI.newButton(frame, pX, pY, frame.size, pTexture, pTip, oTemplate)
		return frame.buttons[pName]
	end
	
	frame.movButton = function(pName, pX, pY, pSize, pTip, oFrame)
		if(frame.buttons[pName] ~= nil) then frame.buttons[pName]:Hide() end
		frame.buttons[pName] = SynthiqBotsUI.movButton(frame, pX, pY, pSize, pTip, oFrame)
		return frame.buttons[pName]
	end
	
	frame.boxButton = function(pName, pX, pY, pSize, pState)
		if(frame.buttons[pName] ~= nil) then frame.buttons[pName]:Hide() end
		frame.buttons[pName] = SynthiqBotsUI.boxButton(frame, pX, pY, pSize, pState)
		return frame.buttons[pName]
	end
	
	frame.catButton = function(pName, pX, pY, pWidth, pHeight)
		if(frame.buttons[pName] ~= nil) then frame.buttons[pName]:Hide() end
		frame.buttons[pName] = SynthiqBotsUI.catButton(frame, pX, pY, pWidth, pHeight)
		return frame.buttons[pName]
	end
	
	frame.addFrame = function(pName, pX, pY, oSize, oWidth, oHeight)
		if(frame.frames[pName] ~= nil) then frame.frames[pName]:Hide() end
		frame.frames[pName] = SynthiqBotsUI.newFrame(frame, pX, pY, SynthiqBotsUI.IF(oSize ~= nil, oSize, frame.size - 4), oWidth, oHeight)
...
SynthiqBotsUI.newButton = function(pParent, pX, pY, pSize, pTexture, pTip, oTemplate)
	local button = CreateFrame("Button", nil, pParent, SynthiqBotsUI.IF(oTemplate ~= nil, oTemplate, "ActionButtonTemplate"))
	button:SetPoint("BOTTOMRIGHT", pX, pY)
	button:SetSize(pSize, pSize)
	button:Show()
	
	button.icon = button:CreateTexture(nil, "BACKGROUND")
	button.icon:SetTexture(SynthiqBotsUI.IF(string.sub(pTexture, 1, 9) ~= "Interface", "Interface/Icons/", "") .. pTexture)
	button.icon:SetAllPoints(button)
	button.icon:Show()
	
	button.border = button:CreateTexture(nil, "ARTWORK")
	button.border:SetTexture("Interface\\AddOns\\synthiqbots-ui\\Icons\\border.blp")
...
SynthiqBotsUI.wowButton = function(pParent, pName, pX, pY, pWidth, pHeight, pSize)
...
SynthiqBotsUI.movButton = function(pParent, pX, pY, pSize, pTip, oFrame)
...
SynthiqBotsUI.boxButton = function(pParent, pX, pY, pSize, pState)
...
SynthiqBotsUI.catButton = function(pParent, pX, pY, pWidth, pHeight)
...
SynthiqBotsUI.addFrame = function(pName, pX, pY, pSize)
	local tFrame = SynthiqBotsUI.newFrame(SynthiqBotsUI, pX, pY, pSize)
	SynthiqBotsUI.frames[pName] = tFrame
	return tFrame
end

SynthiqBotsUI.addSelf = function(pClass, pName)
	if(SynthiqBotsUI.frames["MultiBar"].frames["Units"].buttons[pName] ~= nil) then return SynthiqBotsUI.frames["MultiBar"].frames["Units"].buttons[pName] end
	local tClass = SynthiqBotsUI.toClass(pClass)
	local tButton = SynthiqBotsUI.frames["MultiBar"].frames["Units"].addButton(pName, 0, 0, "inv_misc_head_clockworkgnome_01", SynthiqBotsUI.tips.unit.selfbot)
	if(SynthiqBotsUI.index.classes.players[tClass] == nil) then SynthiqBotsUI.index.classes.players[tClass] = {} end
	table.insert(SynthiqBotsUI.index.classes.players[tClass], pName)
	table.insert(SynthiqBotsUI.index.players, pName)
...
SynthiqBotsUI.addPlayer = function(pClass, pName)
	if(SynthiqBotsUI.frames["MultiBar"].frames["Units"].buttons[pName] ~= nil) then return SynthiqBotsUI.frames["MultiBar"].frames["Units"].buttons[pName] end
	local tClass = SynthiqBotsUI.toClass(pClass)
	local tTexture = "Interface\\AddOns\\synthiqbots-ui\\Icons\\class_" .. string.lower(tClass) .. ".blp"
	local tButton = SynthiqBotsUI.frames["MultiBar"].frames["Units"].addButton(pName, 0, 0, tTexture, SynthiqBotsUI.toTip(tClass, nil, pName))
	if(SynthiqBotsUI.index.classes.players[tClass] == nil) then SynthiqBotsUI.index.classes.players[tClass] = {} end
	table.insert(SynthiqBotsUI.index.classes.players[tClass], pName)
	table.insert(SynthiqBotsUI.index.players, pName)
	tButton.roster = "players"
...
SynthiqBotsUI.addMember = function(pClass, pLevel, pName)
	if(SynthiqBotsUI.frames["MultiBar"].frames["Units"].buttons[pName] ~= nil) then return SynthiqBotsUI.frames["MultiBar"].frames["Units"].buttons[pName] end
	local tClass = SynthiqBotsUI.toClass(pClass)
	local tTexture = "Interface\\AddOns\\synthiqbots-ui\\Icons\\class_" .. string.lower(tClass) .. ".blp"
	local tButton = SynthiqBotsUI.frames["MultiBar"].frames["Units"].addButton(pName, 0, 0, tTexture, SynthiqBotsUI.toTip(tClass, pLevel, pName))
	if(SynthiqBotsUI.index.classes.members[tClass] == nil) then SynthiqBotsUI.index.classes.members[tClass] = {} end
	table.insert(SynthiqBotsUI.index.classes.members[tClass], pName)
	table.insert(SynthiqBotsUI.index.members, pName)
...
SynthiqBotsUI.addFriend = function(pClass, pLevel, pName)
	if(SynthiqBotsUI.frames["MultiBar"].frames["Units"].buttons[pName] ~= nil) then return SynthiqBotsUI.frames["MultiBar"].frames["Units"].buttons[pName] end
	local tClass = SynthiqBotsUI.toClass(pClass)
	local tTexture = "Interface\\AddOns\\synthiqbots-ui\\Icons\\class_" .. string.lower(tClass) .. ".blp"
	local tButton = SynthiqBotsUI.frames["MultiBar"].frames["Units"].addButton(pName, 0, 0, tTexture, SynthiqBotsUI.toTip(tClass, pLevel, pName))
	if(SynthiqBotsUI.index.classes.friends[tClass] == nil) then SynthiqBotsUI.index.classes.friends[tClass] = {} end
	table.insert(SynthiqBotsUI.index.classes.friends[tClass], pName)
	table.insert(SynthiqBotsUI.index.friends, pName)
...
SynthiqBotsUI.addActive = function(pClass, pLevel, pName)
	if(SynthiqBotsUI.frames["MultiBar"].frames["Units"].buttons[pName] ~= nil) then return SynthiqBotsUI.frames["MultiBar"].frames["Units"].buttons[pName] end
	local tClass = SynthiqBotsUI.toClass(pClass)
	local tTexture = "Interface\\AddOns\\synthiqbots-ui\\Icons\\class_" .. string.lower(tClass) .. ".blp"
	local tButton = SynthiqBotsUI.frames["MultiBar"].frames["Units"].addButton(pName, 0, 0, tTexture, SynthiqBotsUI.toTip(tClass, pLevel, pName))
...
SynthiqBotsUI.getBot = function(pName)
	return SynthiqBotsUI.frames["MultiBar"].frames["Units"].buttons[pName]
end