...
//
// `templ` is wrapped in fmt::runtime(...) to opt out of compile-time
// format-string checking — required since AzerothCore bumped fmt to a version
// where `fmt::format(<runtime string>, ...)` is rejected via consteval. Any
// runtime-built format string (DB-loaded prompts, config-driven templates)
// must use fmt::runtime() or the build fails with "call to consteval function
// is not a constant expression". The try/catch still covers the parse-error
// case at runtime.
